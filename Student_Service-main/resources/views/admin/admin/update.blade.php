            ...
            <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto p-10 lg:py-0">
                ...
                <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form class="space-y-4 md:space-y-6" method="POST" action="{{route("admin.admin.update")}}" enctype="multipart/form-data">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="id" value="{{$admin->id}}"/>
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Username</label>
                                <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Username" required value='{{$admin->user->name}}'>
                                @error("name")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
                            <div>
                                <img class="rounded-full w-10 h-10" src="{{ asset( 'storage/'. $admin->user->userProfile->profile) }}" alt="image description">
                            ...
                            <div>
                                ...
                                <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required  value='{{$admin->user->email}}'>
                                @error("email")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
