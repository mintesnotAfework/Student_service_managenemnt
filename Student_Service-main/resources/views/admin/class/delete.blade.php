        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="post" action={{route("admin.class.destory")}}>
                        @csrf
                        @method("delete")
                        <input type="hidden" name="id" value="{{$class->id}}">
                        <div class="border-2 p-3 rounded-lg">
                            <div>
                                <img class="rounded-full w-10 h-10" src="{{ asset('storage/'.$class->building->picture)}}" alt="image description">
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="picture">Class Name : {{$class->name}}</label>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="picture">Building : {{$class->building->name}}</label>
                            </div>
                        </div>
