                ...
                <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                            Update Place On The System
                        </h1>
                        <form class="space-y-4 md:space-y-6" method="POST" action="{{route("admin.place.update")}}"  enctype="multipart/form-data">
                            @csrf
                            @method("put")
                            <input type="hidden" name="id" value="{{$place->id}}"/>
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">name</label>
                                <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Jan Movcov" required value="{{$place->name}}">
                                @error("name")
                            ...
                            <div>
                                ...
                                <input type="url" name="map_url" id="map_url" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="https://map.gogole.com/palce_location" required value='{{$place->map_url}}'>
                                @error("map_url")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
                            <div>
                                <img class="rounded-full w-10 h-10" src="{{ asset( 'storage/'. $place->picture) }}" alt="image description">
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="picture">Picture</label>
                                <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="picture" name="picture" type="file" accept="image/*" value="{{$place->picture}}">
                                @error("picture")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
                        ...
                        <a href="{{route("admin.place.delete",$place->id)}}">
