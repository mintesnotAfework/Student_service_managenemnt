                ...
                <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                            Update an Student account
                        </h1>
                        <form class="space-y-4 md:space-y-6" method="POST" action="{{route("admin.student.update")}}" enctype="multipart/form-data">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="id" value="{{$student->id}}"/>
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Username</label>
                                <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Username" required value='{{$student->user->name}}'>
                                @error("name")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
                            <div>
                                <img class="rounded-full w-10 h-10" src="{{ asset( 'storage/'. $student->user->userProfile->profile) }}" alt="image description">
                            ...
                            <div>
                                ...
                                <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required  value='{{$student->user->email}}'>
                                @error("email")
                                    <p class="text-red-500">{{$message}}</p>
                                @enderror
                            </div>
                            <div>
                                <label for="dorm" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Dorm</label>
                                <select name="dorm" id="dorm" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    @foreach ($dorms as $dorm)
                                        @if($student->dorm_id == $dorm->id)
                                            <option selected value="{{$dorm->id}}">{{$dorm->building->name}} - {{$dorm->name}}</option>
                                        @else
                                            <option value="{{$dorm->id}}">{{$dorm->building->name}} - {{$dorm->name}}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error("dorm")
                            ...
                            <div>
                                ...
                                <select name="field" id="field" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    @foreach ($fields as $field)
                                        @if ($student->field_id == $field->id)
                            ...
                            <div>
                                ...
                                <select name="parent" id="parent" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    @foreach ($parents as $parent)
                                        @if($student->parent_id == $parent->id)
                                            <option selected value="{{$parent->id}}">{{$parent->user->userProfile->full_name}}</option>
                                        @else
                                            <option value="{{$parent->id}}">{{$parent->user->userProfile->full_name}}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error("parent")
                            ...
                            <button type="submit" class="w-full text-grey-100 bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update an Student account</button>
                        </form>
